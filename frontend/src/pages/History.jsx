...
export function History({ searchHistory = [], onSelectCity, onClearHistory }) {
  return (
    <div className="page-fade-in" style={{ maxWidth: '800px', margin: '0 auto' }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '1.5rem' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
          ...
          <h1 style={{ fontSize: '1.6rem', fontWeight: 800 }}>Search & Query History</h1>
        </div>
      ...
      {searchHistory.length === 0 ? (
        <EmptyState
          ...
          description="Your search and location query history will be displayed here for quick access."
          icon={HistoryIcon}
        />
      ) : (
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
                  ...
                  <div>
                    ...
                    <span style={{ fontSize: '0.75rem', color: 'var(--text-muted)' }}>
                      {item.country} • Searched: {new Date(item.timestamp).toLocaleString()}
                    </span>
                  </div>
                </div>
